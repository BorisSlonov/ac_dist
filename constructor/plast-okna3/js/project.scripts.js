...
$(document).ready(function(){
	...
	$('input.custom-upload').each(function(index, element) {
		...
		txt='Прикрепить файл';
		if($(element).attr('accept')=='image/jpeg'){txt='Добавить фото';}
		$(element).after('<a class="upl-text">'+txt+'</a>')
	}).change(function(e) {
		if($(this).attr('accept')=='image/jpeg'){
			$(this).parent().find('.upl-text').text($(this).val() ? 'Фото добавлено' : 'Добавить фото');}
		else{
			$(this).parent().find('.upl-text').text($(this).val() ? 'Файл прикреплен' : 'Прикрепить файл');
		}
	});
